...
export function Footer() {
  return (
    <footer className="mt-auto border-t border-border bg-hearth-deep/50">
      <div className="mx-auto max-w-6xl px-5 py-12">
        <MandalaDivider />

        <div className="mt-10 grid gap-10 md:grid-cols-3">
          <div>
            <p className="font-serif text-xl font-semibold text-depth">
              {siteConfig.name}
            </p>
            ...
            <ul className="mt-3 space-y-2">
              {navItems.map((item) => (
                <li key={item.href}>
                  <Link
                    href={item.href}
                    className="text-sm text-ink-muted transition-colors hover:text-depth"
                  >
                    {item.label}
                  </Link>
                </li>
          ...
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm text-ink-muted">
              ...
              <li>
                <WhatsAppLink className="inline-flex items-center gap-1.5 transition-colors hover:text-sage">
